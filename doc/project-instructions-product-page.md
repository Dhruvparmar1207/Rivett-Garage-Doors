...
When creating or updating residential garage door product pages, use:

`\rivett\Product-Page.php`

as the **MASTER PAGE LAYOUT/TEMPLATE**.

The new page must follow the layout and structure of `Product-Page.php`, including where applicable:
...
**Do not copy the page-specific content from `Product-Page.php` unless specifically requested.**

Use `Product-Page.php` for the **NEW WEBSITE'S LAYOUT**.

Use the reference URL for the **CONTENT**.

Therefore:

**Product-Page.php = PAGE LAYOUT/TEMPLATE**

**Reference website = CONTENT SOURCE**

**Local destination folder = FINAL IMPLEMENTATION**

If the existing `Product-Page.php` layout can be reused directly, reuse its structure/components rather than creating a completely different layout.

If you need to create a new component, it must visually and structurally match the existing `Product-Page.php` page.
