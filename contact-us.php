<?php include 'header.php'; ?>

    <section class="hero banner-sec contact-page-banner rivett-padding" aria-labelledby="hero-title">
        <div class="container-md">
            <div class="row align-items-start">

                <div class="col-lg-6 left-side">
                    <div class="banner-text">
                    <h1 id="hero-title text-black" style="color: #000;">Let’s Get Your Doors Sorted</h1></div>

                    <h3>Reach Us Directly</h3>
                        <div class="main-info-box">
                            <article>
                            <div class="icon"><img src="./src/images/big-call-circle-icon.svg" alt=""
                                    width="54" height="54" loading="lazy"></div>
                            <div class="info">
                                <p>Whitby Main</p>
                                <p><a href="tel:19056684455">1 (905) 668-4455</a></p>
                            </div>
                        </article>

                        <article>
                            <div class="icon"><img src="./src/images/big-call-circle-icon.svg" alt=""
                                    width="54" height="54" loading="lazy"></div>
                            <div class="info">
                                <p>Kingston</p>
                                <p><a href="tel:16133895218">1 (613) 389-5218</a></p>
                            </div>
                        </article>

                        <article>
                            <div class="icon"><img src="./src/images/big-email-circle-icon.svg" alt=""
                                    width="54" height="54" loading="lazy"></div>
                            <div class="info">
                                <p>Email Address</p>
                                <p><a href="mailto:contact@rivett.com">contact@rivett.com</a></p>
                            </div>
                        </article>


                        <article>
                            <div class="icon"><img src="./src/images/big-clock-circle-icon.svg" alt=""
                                    width="54" height="54" loading="lazy"></div>
                            <div class="info">
                                <p>Business Hours (Whitby Main Office)</p>
                                <p class="main-time">
                                    <span class="new-day-time">
                                        <span class="day">Mon – Thu</span>
                                        <span class="jem">:</span>
                                        <span class="time">8:00 AM – 5:00 PM</span>
                                    </span>
                                    <span class="new-day-time">
                                        <span class="day">Friday</span>
                                        <span class="jem">:</span>
                                        <span class="time">8:00 AM – 1:00 PM</span>
                                    </span>
                                </p>
                            </div>
                        </article>
                        </div>


                        <div class="call-popup-emergency">
                            <div class="call-popup-emergency-inner">
                                <h3>Overhead Door Emergency Service</h3>
                               
                                <a href="tel:19054242512" class="call-location-num">
                               <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M4.86349 15.9536C7.23964 18.7833 10.1 21.0112 13.3647 22.5877C14.6077 23.1745 16.27 23.8707 18.122 23.9901C18.2368 23.995 18.3467 24 18.4615 24C19.7045 24 20.7028 23.5723 21.5165 22.6921C21.5215 22.6871 21.5315 22.6772 21.5365 22.6672C21.826 22.3191 22.1555 22.0058 22.4999 21.6726C22.7345 21.4488 22.9742 21.2151 23.2038 20.9764C24.2671 19.8724 24.2671 18.47 23.1938 17.4007L20.1937 14.4119C19.6845 13.8848 19.0755 13.6063 18.4365 13.6063C17.7975 13.6063 17.1835 13.8848 16.6594 14.407L14.8723 16.1873C14.7075 16.0928 14.5378 16.0083 14.3781 15.9287C14.1784 15.8293 13.9937 15.7348 13.829 15.6303C12.2016 14.6009 10.724 13.2582 9.31129 11.5325C8.59744 10.6324 8.11822 9.8765 7.78376 9.10568C8.253 8.68297 8.69229 8.24037 9.1166 7.80771C9.26636 7.65354 9.42111 7.49938 9.57586 7.34521C10.115 6.80812 10.4045 6.18649 10.4045 5.55491C10.4045 4.92333 10.12 4.3017 9.57586 3.76461L8.08827 2.28264C7.91355 2.10858 7.74882 1.93949 7.57909 1.76544C7.24963 1.42727 6.90518 1.07915 6.56573 0.765852C6.05157 0.263572 5.44755 0 4.80858 0C4.17461 0 3.56559 0.263572 3.03146 0.770825L1.16448 2.63075C0.485583 3.30709 0.101206 4.12764 0.021335 5.0775C-0.0735114 6.26606 0.146133 7.52922 0.715211 9.05595C1.5888 11.4182 2.90666 13.6113 4.86349 15.9536ZM1.23936 5.18193C1.29927 4.52051 1.55385 3.9685 2.03308 3.49109L3.89007 1.64111C4.1796 1.36262 4.49908 1.2184 4.80858 1.2184C5.11309 1.2184 5.42259 1.36262 5.70713 1.65106C6.04158 1.95939 6.35607 2.28264 6.69552 2.62578C6.86525 2.79983 7.03997 2.97389 7.21468 3.15292L8.70227 4.63489C9.01177 4.94322 9.17151 5.25653 9.17151 5.56486C9.17151 5.87319 9.01177 6.18649 8.70227 6.49482C8.54752 6.64898 8.39278 6.80812 8.23803 6.96229C7.77378 7.42976 7.33948 7.87236 6.86026 8.29507L6.8353 8.31993C6.42097 8.7327 6.48586 9.12557 6.5857 9.42395C6.59069 9.43887 6.59569 9.44882 6.60068 9.46374C6.98506 10.3838 7.51919 11.259 8.35284 12.3034C9.85041 14.1434 11.4279 15.5707 13.165 16.6697C13.3797 16.809 13.6093 16.9184 13.824 17.0278C14.0237 17.1272 14.2084 17.2217 14.3731 17.3261C14.3931 17.3361 14.408 17.346 14.428 17.356C14.5927 17.4405 14.7525 17.4803 14.9122 17.4803C15.3116 17.4803 15.5711 17.2267 15.656 17.1421L17.523 15.2822C17.8125 14.9938 18.127 14.8396 18.4365 14.8396C18.8159 14.8396 19.1254 15.0734 19.3201 15.2822L22.3302 18.276C22.9292 18.8728 22.9242 19.5193 22.3152 20.1509C22.1056 20.3746 21.8859 20.5885 21.6513 20.8123C21.3019 21.1504 20.9375 21.4986 20.608 21.8914C20.0339 22.5081 19.35 22.7965 18.4665 22.7965C18.3816 22.7965 18.2917 22.7915 18.2069 22.7866C16.5695 22.6821 15.047 22.0456 13.9038 21.5035C10.7989 20.0066 8.07329 17.8831 5.81196 15.1877C3.94997 12.9548 2.697 10.8761 1.86834 8.64816C1.35418 7.28056 1.15949 6.18152 1.23936 5.18193Z" fill="white"/>
                                </svg>1 (905) 424-2512
                                </a>
                            </div>
                        </div>



                </div>

            <div class="col-lg-6 right-side contact-form-sec">

                    <div class="contact-page-form">
                          <h3>Send Us a Message</h3>

                            <form id="ContactusForm" action="" data-parsley-validate="" novalidate="">

                                    <div class="form-row">
                                        <div class="contact-us-form-row first-block">
                                            <input type="text" name="name" placeholder="Name*" required="" data-parsley-required="true" data-parsley-required-message="Please Enter Your Name" data-parsley-errors-container="#errorName" data-parsley-pattern="/^([a-zA-Z]+\s)*[a-zA-Z]+\s*$/" data-parsley-pattern-message="Please Enter Valid Name">
                                            <small id="errorName"></small>
                                        </div>

                                        <div class="contact-us-form-row second-block">
                                            <input type="email" name="email" placeholder="Email*" required="" data-parsley-required="true" data-parsley-required-message="Please enter your Email" data-parsley-type="email" data-parsley-type-message="Please enter a valid email address" data-parsley-errors-container="#errorEmail">
                                            <small id="errorEmail"></small>
                                        </div>

                                        <div class="contact-us-form-row third-block">
                                            <input type="tel" name="phone" placeholder="Phone*" required="" data-parsley-required="true" data-parsley-required-message="Please enter your Phone number" data-parsley-pattern="^[0-9\-\+\s\(\)]+$" data-parsley-pattern-message="Please enter a valid phone number" data-parsley-errors-container="#errorPhone">
                                            <small id="errorPhone"></small>
                                        </div>
                                        <div class="contact-us-form-row fourth-block">
                                            <select class="contact_input" id="service_type" name="service_type" data-parsley-errors-container="#errorservice_type" required="" data-parsley-required-message="Please Select  Inquiry Type">
                                                <option value="">
                                                    Select Service
                                                </option>
                                                <option value="Sales">Sales</option>
                                                <option value="Service &amp; Repair">Service &amp; Repair</option>
                                                <option value="Parts">Parts</option>
                                            </select>
                                            <small id="service_type"></small>
                                            
                                        </div>

                                        <div class="contact-us-form-row fifth-block">
                                            <input type="text" name="city" placeholder="City*" required="" data-parsley-required="true" data-parsley-required-message="Please Enter Your City" data-parsley-errors-container="#errorCity">
                                            <small id="errorCity"></small>
                                        </div>

                                        <div class="contact-us-form-row sixth-block">
                                            <textarea name="message" id="message" placeholder="How Can We Help?" rows="3"></textarea>
                                        </div>
                                    </div>

                                    <button type="submit" class="cmn-btn">Send</button>

                            </form>
                     </div>  

                </div>
            </div>
        </div>
    </section>




<?php include 'common-sections/location-mapping-sec.php'; ?>

<?php include 'footer.php'; ?>